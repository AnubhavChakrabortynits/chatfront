...
<span className='username' style={{margin:'auto'}}>~ {message.name==currentuser?'You':message.name}</span>
<pre className='messagebox bgblue'>
   <span>
       {message.type=='userjoined'?(String((message.name==currentuser?'You ':message.name))+`joined room --${message.room}`):message.mesg}
       </span>  
</pre>
    </div>
    : 
